        ...
        <div class="flex flex-wrap gap-4 items-center">
            ...
            @foreach ($headers as $header)
                @if ($header['filterable'] ?? false)
                    ...
                    @else
                        <input type="{{ $header['filter_type'] ?? 'text' }}" id="filter-{{ $slug }}"
            ...
            @can('viewAdminDashboard')
                <div class="relative ml-auto">
                    ...
                    <div id="export-options"
                        ...
                        <a href="#" data-format="excel"
                            class="block px-4 py-2 text-gray-700 hover:bg-gray-100">Excel</a>
                    ...
                    @else
                        @foreach ($rows as $row)
                            <tr class="hover:bg-gray-50">
                                ...
                                @foreach ($headers as $header)
                                    <td class="px-4 py-2 border capitalize"
                                        data-column="{{ Str::slug($header['label']) }}">
                                        {{ $row[$header['label']] ?? '' }}
                                    </td>
                                @endforeach
                                @if ($actions)
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterRows() {
            ...
            document.querySelectorAll('#table-body tr').forEach(row => {
                ...
                const matchesColumns = filters.every(filter => {
                    ...
                    const cell = cells.find(c => c.getAttribute('data-column') === filter
                        .column);
        ...
        document.getElementById('export-options').addEventListener('click', async (e) => {
            ...
            const visibleRows = Array.from(document.querySelectorAll(
                '#table-body tr:not([style*="display: none"])'));
